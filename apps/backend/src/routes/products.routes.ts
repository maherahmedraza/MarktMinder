...
router.post(
    ...
    asyncHandler(async (req: Request, res: Response) => {
        ...
        const existingTrack = await prisma.userProduct.findUnique({
            where: {
                userId_productId: {
                    userId,
...
router.get(
    ...
    asyncHandler(async (req: Request, res: Response) => {
        ...
        const total = await prisma.userProduct.count({ where });
...
router.get(
    ...
    asyncHandler(async (req: Request, res: Response) => {
        ...
        if (req.user) {
            const track = await prisma.userProduct.findUnique({
                where: {
                    userId_productId: {
                        userId: req.user.id,
                        productId: product.id
