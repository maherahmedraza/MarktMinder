    ...
    try {
        ...
        res.json({
            data: products.map((row: any) => ({
                ...
                currentPrice: Number(row.currentPrice) || null,
    ...
    try {
        ...
        const product = await prisma.product.findUnique({
            ...
            include: {
                priceHistory: {
                    where: {
                        time: { gte: new Date(Date.now() - 30 * 24 * 60 * 60 * 1000) }
                    },
                    orderBy: { time: 'desc' },
                    take: 100
                }
        ...
        res.json({
            data: {
                ...
                currentPrice: Number(product.currentPrice) || null,
                currency: product.currency,
                availability: product.availability,
                lowestPrice: Number(product.lowestPrice) || null,
                highestPrice: Number(product.highestPrice) || null,
                averagePrice: Number(product.averagePrice) || null,
...
router.get('/products/:id/prediction', async (req: Request, res: Response) => {
    try {
        ...
        const prediction = await getPricePrediction(id, days, product.marketplace);
