    ...
    try {
        ...
        if (productId) {
            products = await prisma.product.findMany({
                where: { id: productId },
                select: { id: true, currentPrice: true, title: true }
            });
        } else {
            products = await prisma.product.findMany({
        ...
        logger.info(`Found ${products.length} products to seed with historical data`);

        for (const product of products) {
            const basePrice = Number(product.currentPrice) || 50;
...
// Check if file is being run directly (ESM approach for checking main module is tricky, but check argv)
// or just export it. The previous code ran it if argv passed.
// We can preserve that behavior.
const productIdArg = process.argv[2];
if (process.argv[1].includes('seed-historical-data.ts')) {
