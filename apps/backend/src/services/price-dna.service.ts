...
async function analyzeWeeklyPattern(productId: string): Promise<{
    ...
    const totalAvg = result.reduce((sum: number, r: any) => sum + Number(r.avg_price), 0) / result.length;

    const patterns: DayPattern[] = result.map((row: any) => ({
        day: parseInt(row.day_of_week),
        dayName: DAY_NAMES[parseInt(row.day_of_week)],
        averagePrice: Number(row.avg_price),
        priceChange: ((Number(row.avg_price) - totalAvg) / totalAvg) * 100,
...
async function analyzeMonthlyPattern(productId: string): Promise<{
    ...
    const patterns: MonthPattern[] = result.map((row: any) => ({
        period: row.period as 'beginning' | 'middle' | 'end',
        averagePrice: Number(row.avg_price),
        priceChange: ((Number(row.avg_price) - totalAvg) / totalAvg) * 100,
        bestBuyWindow: Number(row.avg_price) === Math.min(...result.map((r: any) => Number(r.avg_price))),
    }));
...
async function analyzeSeasonalPattern(productId: string): Promise<{
    ...
    const patterns: SeasonalPattern[] = result.map((row: any) => {
...
export async function analyzePriceDNA(productId: string): Promise<PriceDNA> {
    ...
    const overallAverage = Number(stats.avg_price) || Number(product.currentPrice) || 0;
    ...
    const dna: PriceDNA = {
        ...
        priceRange: {
            min: Number(stats.min_price) || overallAverage,
            max: Number(stats.max_price) || overallAverage,
        },
        totalDataPoints: Number(stats.total_points) || 0,
        dataSpanDays: Math.round(Number(stats.data_span_days) || 0),
