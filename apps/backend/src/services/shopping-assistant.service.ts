...
export async function getBuyWaitRecommendation(productId: string): Promise<BuyWaitRecommendation> {
    ...
    try {
        ...
        const currentPrice = latestHistory?.price ? Number(latestHistory.price) : (Number(product.currentPrice) || 0);
        ...
        const [prediction, discountInfo, buyingWindows, priceStats] = await Promise.all([
            ...
            discountAnalysis.analyzeDiscount(productId, currentPrice, undefined).catch(() => null),
...
async function getProductPriceStats(productId: string): Promise<{
    ...
    const lowestDateEntry = await prisma.priceHistory.findFirst({
        where: { productId },
        orderBy: { price: 'asc' },
        select: { time: true }
    });

    const daysSinceLowest = lowestDateEntry?.time
        ? Math.floor((Date.now() - new Date(lowestDateEntry.time).getTime()) / (1000 * 60 * 60 * 24))
        : null;

    return {
        minPrice: Number(stats._min.price) || 0,
        maxPrice: Number(stats._max.price) || 0,
        avgPrice: Number(stats._avg.price) || 0,
        recordCount: Number(stats._count) || 0,
        daysSinceLowest,
    };
}
