...
export async function getPricePrediction(
    ...
    const pricePoints: PricePoint[] = history.map((row: any) => ({
...
export async function getTrendingProducts(limit: number = 5): Promise<any[]> {
    // Since we joined user_products, which has a Prisma model used by other parts, we know this is a safe basic query.
    // However, the COUNT/GROUP BY logic is easier in raw SQL for now.
    // We can also use prisma.product.findMany with include _count but that counts ALL trackers, not recently.
    // The query groups by p.id and orders by count.
