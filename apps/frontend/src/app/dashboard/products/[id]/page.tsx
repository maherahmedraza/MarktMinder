...
import { NLPAlertInput } from '@/components/alerts/NLPAlertInput';
...
export default function ProductDetailPage() {
    ...
    useEffect(() => {
        ...
        return () => {
            if (socket) socket.off('product:updated', handleUpdate);
        };
    }, [socket, productId]);
    ...
    async function handleAddToFolder(folderId: string) {
        ...
        try {
            const response = await fetch(`/api/folders/products`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ folderId, productId }),
                credentials: 'include'
            });
            if (!response.ok) throw new Error('Failed to update folder');
            toast.success('Asset moved to collection');
            loadProduct(); // Refresh to show new folder name if displayed
        } catch (err) {
            toast.error('Could not move asset');
        } finally {
                ...
                <div className="flex-1 space-y-6">
                    <div>
                        ...
                        <div className="flex items-center gap-3 mt-4">
                            <div className="flex items-center gap-2 px-3 py-1.5 rounded-xl bg-surface-elevated border border-border/30 shadow-sm transition-all group hover:border-primary/30">
                                <Folder className="w-4 h-4 text-text-tertiary group-hover:text-primary transition-colors" />
                                <div className="flex flex-col">
                                    <span className="text-[8px] font-black text-text-tertiary uppercase tracking-widest leading-none mb-0.5">Collection</span>
                                    <select
                                        className="bg-transparent border-none text-[10px] font-black text-text-primary uppercase tracking-widest focus:ring-0 cursor-pointer outline-none p-0 h-4 min-w-[100px]"
                                        value={(product as any).folder_id || ''}
                                        onChange={(e) => handleAddToFolder(e.target.value)}
                                        disabled={isUpdatingFolder}
                                    >
                                        <option value="" className="bg-surface text-text-primary">Unassigned</option>
                                        {folders.map(f => (
                                            <option key={f.id} value={f.id} className="bg-surface text-text-primary">{f.name}</option>
                                        ))}
                                    </select>
                                </div>
                                {isUpdatingFolder && <Loader2 className="w-3 h-3 animate-spin text-primary" />}
                            </div>

                            <button
                                onClick={() => router.push('/dashboard/products')} // Actually we should have a link to the folder or asset ledger with filter
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div className="lg:col-span-2">
                    <NLPAlertInput
                        productId={productId}
                        onAlertCreated={() => {
                            toast.success('AI Alert System Initialized');
                            // Refresh alerts or other state if needed
                        }}
                    />
                </div>
